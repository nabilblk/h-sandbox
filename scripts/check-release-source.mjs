...
export function releaseSource(input, cwd = process.cwd()) {
  ...
  const packageSet = input.packageSet || "core";
  assert.ok(["core", "deepagents"].includes(packageSet), "Unknown release package set");
  const tagPrefix = packageSet === "deepagents" ? "deepagents-v" : "v";
  const ref = input.sourceRef || input.workflowSha;
  const isTag = typeof ref === "string" && ref.startsWith(tagPrefix) && versionPattern.test(ref.slice(tagPrefix.length));
  ...
  let version;
  if (packageSet === "deepagents") {
    assert.ok(!input.chartVersion && !input.imageTag && !input.component, "Adapter releases cannot publish server artifacts");
    const manifest = JSON.parse(git("show", `${sha}:packages/deepagents/package.json`));
    assert.equal(manifest.name, "@h-sandbox/deepagents");
    assert.notEqual(manifest.private, true, "Adapter is not qualified for publication");
    assert.equal(manifest.publishConfig?.access, "public", "Adapter must be public");
    assert.ok(versionPattern.test(manifest.peerDependencies?.["@h-sandbox/sdk"] ?? ""), "Adapter requires an exact released SDK peer");
  ...
  if (isTag) assert.equal(ref, `${tagPrefix}${version}`, "Source tag does not match package version");
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  try {
    const result = releaseSource({ repository: process.env.GITHUB_REPOSITORY, allowedRepository: process.env.RELEASE_REPOSITORY,
      ...
      packageSet: process.env.RELEASE_PACKAGE_SET });
