...
export const config = {
  ...
  persistentWorkspacesEnabled: process.env.PERSISTENT_WORKSPACES_ENABLED === "1",
  workspaceStorageClass: process.env.WORKSPACE_STORAGE_CLASS || null,
  workspaceSizeGiB: Number(process.env.WORKSPACE_SIZE_GIB ?? 10),
  workspaceMaxPerOrganization: Number(process.env.WORKSPACE_MAX_PER_ORGANIZATION ?? 20),
  ...
  sandboxRouteDefaultAccessMode: process.env.SANDBOX_ROUTE_DEFAULT_ACCESS_MODE ?? "public",
  ...
  sandboxCommandTimeoutMs: Number(process.env.SANDBOX_COMMAND_TIMEOUT_MS ?? 30_000),
  sandboxRuntimeUser: process.env.SANDBOX_RUNTIME_USER ?? "root",
  sandboxRuntimeShell: process.env.SANDBOX_RUNTIME_SHELL ?? "/bin/sh",
  ...
  usageObserverEnabled: bool(process.env.USAGE_OBSERVER_ENABLED, true),
  usageRetentionDays: Number(process.env.USAGE_HISTORY_RETENTION_DAYS ?? 30),
  operatorMetricsEnabled: bool(process.env.OPERATOR_METRICS_ENABLED, false),
  operatorMetricsHost: process.env.OPERATOR_METRICS_HOST ?? "127.0.0.1",
  operatorMetricsPort: Number(process.env.OPERATOR_METRICS_PORT ?? 9130),
  operatorMetricsToken: process.env.OPERATOR_METRICS_TOKEN ?? "",
