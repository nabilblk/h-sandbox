...
npm install --save-dev tsx typescript @types/node` }
    ...
    <p>The Python task prints <code>4</code>. The SDK asserts the exit code and exact output, and prints PASS only after cleanup confirmation. Its explicit helper also works with rc.10 APIs; the <a href="#docs/typescript-sdk?section=failure-and-cleanup">rc.11 SDK</a> adds <code>kill({"{ wait: true }"})</code>. The shell example requests deletion. In the dashboard's sandbox history, confirm the runtime is terminated. The CLI's progress messages include the sandbox ID if you need to investigate.</p>
