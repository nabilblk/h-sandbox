...
const QuickstartBody = () => {
  ...
  return <>
    ...
    <p>The examples below pin <code>0.5.0-rc.5</code>, the recorded Developer Preview. Confirm your operator runs the matching API. Unversioned installs still select the older <code>0.4.0</code> stable channel. Review <a href="#docs/developer-preview">version and runtime limits</a> first.</p>
...
# HARAKIRI_API_KEY must already be set privately.`}</CodeBlock>
    <CodeTabs label="Install method" value={method} onValueChange={setMethod} examples={[
      { label: "CLI", language: "bash", code: `npm install -g @h-sandbox/cli@0.5.0-rc.5
