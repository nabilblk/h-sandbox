...
harakiri run "$SBX_ID" --cwd /workspace \\
...
export const overviewDocs: DocPage = {
  ...
  body: <>
    ...
    <a className="docs-start-link" href="#docs/quickstart"><span><strong>Run your first task</strong><small>CLI or TypeScript SDK / about 5 minutes</small></span><Icon name="arrowR" size={20} /></a>
    ...
    <aside className="docs-notice"><p><strong>Choose your release deliberately.</strong> Stable SDK and CLI packages are on npm <code>latest</code> (0.4.0). Workspace and command-output additions are on <code>next</code> (0.5.0-rc.3), with matching server requirements. Preview pages document limitations and prerequisites; they are not features of every deployment.</p><p><a href="#changelog">Read the release notes</a></p></aside>
  </>
};
...
const QuickstartBody = () => {
  ...
  return <>
    ...
    <p>The examples below pin stable <code>0.4.0</code>. For a <code>0.5.0-rc.3</code> deployment, install that version instead. The preview-specific prerequisites are in the <a href="#docs/workspace-reference">workspace reference</a>.</p>
...
# HARAKIRI_API_KEY must already be set privately.`}</CodeBlock>
    <CodeTabs label="Install method" value={method} onValueChange={setMethod} examples={[
      { label: "CLI", language: "bash", code: `npm install -g @h-sandbox/cli@0.4.0
