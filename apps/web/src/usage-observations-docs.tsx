...
curl --fail-with-body --get "$HARAKIRI_API_URL/v1/usage/history" \\
  ...
  --data-urlencode "resolution=1m"`}</CodeBlock>
    ...
    <p><code>400 validation_error</code> means invalid parameters or too many buckets. <code>403 forbidden</code> requires correct organization access or key scope. A <code>404</code> on an older server means history is unsupported. <code>503 usage_history_limit_exceeded</code> calls for a shorter window. <code>503 usage_history_unavailable</code> means a read failed or exceeded its budget; retry the read, not the original sandbox command.</p>
