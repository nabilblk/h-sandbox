...
try {
  ...
  await workspace.archive();
    ...
    <aside className="docs-notice"><p><strong>Unreleased request controls.</strong> The next source revision adds <code>requestTimeoutMs</code> to client configuration and primary command, file and lifecycle calls. Its default is 120 seconds per JSON request, including body consumption. <code>HarakiriRequestTimeoutError</code> reports a local timeout, not a remote failure, and no mutation is automatically retried. These controls are not in the published rc.11 package. Streaming and route requests retain their existing separate controls. See <a href="#docs/deepagents">persistent framework workflows</a> for recovery and approval boundaries.</p></aside>
    <h2>Compatibility</h2>
