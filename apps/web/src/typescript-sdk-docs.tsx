...
export const typescriptSdkDocs: DocPage = {
  ...
  body: <>
    <h2>Availability</h2>
    <aside className="docs-notice"><p><strong>TypeScript SDK 0.5.0-rc.12.</strong> These conveniences require <code>@h-sandbox/sdk@0.5.0-rc.12</code>; npm rc.10 does not include them. This remains a Developer Preview on the <code>next</code> channel, not a stable-release promotion. See the <a href="https://github.com/nabilblk/h-sandbox/releases/tag/v0.5.0-rc.12">release notes</a>.</p></aside>
    <p>Use Node.js 22 LTS for the published SDK preview. Source compatibility checks also cover Node 24 LTS; Node 20 is a legacy check, not a recommendation for new applications. The SDK targets Harakiri API 0.5.0-rc.9 or newer with execution readiness and capacity admission. Usage history additionally requires rc.10 or newer. Templates must exist in your installation; retained workspaces also require operator-enabled storage. This is a server-side SDK, not a way to expose control-plane keys to a browser.</p>
    <h2>Connect</h2>
    ...
    <CodeBlock language="bash" filename="Install the SDK">{`npm install --save-exact @h-sandbox/sdk@0.5.0-rc.12
...
try {
  ...
  await workspace.archive();
    ...
    <p>For framework-led agents, the optional <a href="#docs/deepagents">Deep Agents and LangGraph integration</a> supplies sandbox tools over this SDK. Install its independently versioned npm preview separately; it does not add framework dependencies to the core SDK.</p>
    ...
    <aside className="docs-notice"><p><strong>Bounded request controls in rc.12.</strong> Configure <code>requestTimeoutMs</code> on the client or primary command, file and lifecycle calls. The default is 120 seconds per JSON request, including body consumption. <code>HarakiriRequestTimeoutError</code> reports a local timeout, not a remote failure, and no mutation is automatically retried. SDK rc.11 does not include these controls. For long foreground work, allow enough request time or use detached commands. Streaming and route requests retain their existing separate controls. See <a href="#docs/deepagents">persistent framework workflows</a> for recovery and approval boundaries.</p></aside>
    <h2>Compatibility</h2>
    ...
    <p>The <a href="https://github.com/nabilblk/h-sandbox/actions/workflows/sdk-acceptance.yml">SDK acceptance workflow</a> is configured to test installed tarballs on Node 22/24 and legacy Node 20, then run model-free workflows against a disposable native amd64 installation. A passing run is required evidence, not a claim that every provider or LLM was tested. Package publication and independent integration feedback remain separate release gates.</p>
