...
export const previewDocs: DocPage = {
  ...
  body: <>
    ...
    <p>The current candidate is <code>0.5.0-rc.12</code>. SDK and CLI packages use the <code>next</code> channel; <code>latest</code> still identifies the older <code>0.4.0</code> release. The optional <a href="#docs/deepagents">Deep Agents adapter</a> is independently versioned and available on npm through its own <code>next</code> channel.</p>
    <CodeBlock language="bash">{`npm install --save-exact @h-sandbox/sdk@0.5.0-rc.12
npm install -g @h-sandbox/cli@0.5.0-rc.12
harakiri --version`}</CodeBlock>
    <p>For installation and upgrade, use the <a href="https://github.com/nabilblk/h-sandbox/releases/tag/v0.5.0-rc.12">0.5.0-rc.12 release notes and artifact receipt</a>. The Kubernetes guide preserves its qualified rc.9 bootstrap and rc.10 upgrade. rc.12 adds request deadlines and framework recovery documentation without a database migration. Match API, scheduler, web and chart versions for a full installation. Migration 038 still requires coordinated activation when upgrading from pre-capacity releases. Existing videos record the version shown in their evidence, not every later change.</p>
