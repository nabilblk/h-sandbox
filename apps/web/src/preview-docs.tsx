...
export const previewDocs: DocPage = {
  ...
  body: <>
    ...
    <p>The latest candidate targeted by this guide is <code>0.5.0-rc.6</code>. SDK and CLI packages are on the <code>next</code> channel; <code>latest</code> still identifies the older <code>0.4.0</code> release. Pin the version agreed with your operator instead of mixing channels.</p>
    <CodeBlock language="bash">{`npm install --save-exact @h-sandbox/sdk@0.5.0-rc.6
npm install -g @h-sandbox/cli@0.5.0-rc.6
harakiri --version`}</CodeBlock>
    <p>For rc.5 installation and upgrade, use its <a href="https://github.com/nabilblk/h-sandbox/blob/main/docs/release-notes/0.5.0-rc.6-delivery.md">delivery receipt</a> and exact values overlay. Match the API, web, chart and package versions to the receipt. Source changes after that receipt are unreleased until a new candidate is published. Existing videos record the version shown in their evidence, not every later change.</p>
