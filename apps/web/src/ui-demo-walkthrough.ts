...
export const uiDemoSteps: DemoStep[] = [
  {
    ...
    text: 'A developer wants a release-status prototype without setting up a local toolchain. Use the dashboard to create a workspace, ask OpenCode to build the app, inspect its files and expose a temporary preview. This is a prototype with synthetic releases, not a production deployment.', code: '',
  ...
  },
  {
    ...
    text: 'Your browser controls Harakiri through its authenticated dashboard. The terminal, Node server and agent run in the sandbox. OpenCode calls an external free-model provider. Only synthetic data is used, and no paid provider credentials are supplied.', code: '',
  ...
  },
  {
    ...
    code: 'OPENCODE_CONFIG_CONTENT={"model":"opencode/mimo-v2.5-free","small_model":"opencode/mimo-v2.5-free","enabled_providers":["opencode"],"share":"disabled","autoupdate":false,"permission":{"*":"allow"}}',
  ...
  },
  {
    ...
    text: 'The prompt has two parts, both shown in full here and in ui-prompt.txt. This first part defines the output files, HTTP binding, health contract and exact filter behavior. Define these acceptance criteria before the agent begins.', code: uiPromptParts[0]!,
  ...
  },
  {
    ...
    text: 'The second part specifies presentation, live row count and a syntax check. It asks the agent to write and test files but leave server startup to the operator. The complete command in the next chapter joins both parts; there is no hidden FULL_PROMPT placeholder.', code: uiPromptParts[1]!,
  ...
  },
  {
    ...
    code: `cd /workspace && opencode run --format json --model opencode/mimo-v2.5-free '${uiDemoPrompt}' > agent.jsonl 2>&1`,
  ...
  },
  {
    ...
    code: `jq -Rr 'fromjson? | select(.type == "tool_use") | [.part.tool,.part.state.status] | @tsv' agent.jsonl\njq -Rr 'fromjson? | select(.type == "step_finish") | .part.cost' agent.jsonl`,
  ...
  },
  {
    ...
    text: 'Open Filesystem and inspect /workspace. Confirm server.mjs and index.html exist. Review their contents before exposing them. These are agent-written files from this run; no finished application was uploaded. Download any work you want to keep before termination.', code: '',
  ...
  },
  {
    ...
    text: 'Open Network. Enter 3000, select HTTP and Public, then Expose port. Open the generated URL. Public means anyone with the URL can reach the app; use only synthetic content. Never expose the agent control server or a shell as an unauthenticated public service.', code: '',
  ...
  },
  {
    ...
    text: 'Open the preview in a browser. Confirm all three releases appear. Select passed and require Portal only, then select All to restore three rows. Inspect the visible count as well. The recording independently asserted the row/filter behavior rather than trusting agent output; generated design may differ in your run.', code: '',
  ...
  },
  {
    ...
    text: 'Return to the sandbox detail, retain any files, and select Kill. Wait for terminated, then refresh the preview URL and confirm it no longer serves the app. If connectivity fails, use the retained sandbox ID to recover in Sandboxes. TTL is an additional safety net.', code: '',
