  ...
  },
  {
    ...
    body: (
      <>
        ...
        <p>The <a href="#docs/typescript-sdk">TypeScript SDK guide</a> covers task-oriented execution, process reconnect, native text/bytes and confirmed cleanup introduced in rc.11. The lower-level response-envelope methods below remain supported.</p>
  ...
  },
  {
    ...
    body: (
      <>
        ...
        <p><strong>From 0.5.0-rc.10:</strong> <code>harakiri usage --period 24h|7d|30d --json</code> reads historical observations. It requires the matching history API and <code>org:read</code>; it is not included in rc.9. See <a href="#docs/usage-observations">Usage observations</a>.</p>
        <h2>Lifecycle</h2>
...
const { result } = await client.runSandbox(sandboxId, {
  command: "python /workspace/task.py"
        ...
        <p><strong>From rc.11:</strong> default creation can throw <code>HarakiriSandboxCreationError</code> after acknowledgement. Read <code>sandboxId</code> and <code>cause</code>, not <code>error.id</code>. A direct read-only wait still throws <code>HarakiriWaitTimeoutError</code>. Use <code>kill({"{ wait: true }"})</code> and read-only <code>waitForTermination()</code> to confirm this sandbox's capacity release. See <a href="#docs/typescript-sdk?section=failure-and-cleanup">failure and cleanup</a>; older rc.10 callers need explicit observations.</p>
  ...
  },
  {
    ...
    body: (
      <>
        ...
        <h2>SDK</h2>
        <p>This complete example validates the transfer envelope, bytes and SHA-256. Run it as <code>node artifacts.mjs</code> after <a href="#docs/sdk-cli">installing and configuring the published SDK</a>.</p>
        <CodeBlock language="javascript" filename="artifacts.mjs">{publishedArtifacts}</CodeBlock>
        <p>The <a href="#docs/typescript-sdk?section=files-and-artifacts">rc.11 byte helpers</a> reduce this to <code>files.write(path, bytes)</code> and <code>readBytes(path)</code>, with validation inside the SDK. They are not available in npm rc.10. Raw <code>artifacts.upload/download</code> and object-input file calls remain supported.</p>
        <h2>CLI</h2>
  ...
  },
  {
    ...
    body: (
      <>
        ...
        <h2>SDK</h2>
        <p><strong>rc.11 migration:</strong> the <a href="#docs/typescript-sdk?section=protected-services">scoped route adapter</a> preserves Request semantics and forces manual redirects, even on the existing method below. Health checks no longer silently follow redirects; inspect the response instead. Older rc.10 lacks these safeguards and needs explicit redirect rejection and trusted relative paths.</p>
...
await sandbox.credentials.attachReference(external.reference.id);`}</CodeBlock>
  ...
  },
  {
    ...
    body: (
      <>
        <h2>JavaScript</h2>
        <p>For rc.11's object-oriented workflow and migration notes, read the <a href="#docs/typescript-sdk">TypeScript SDK guide</a>. The lower-level calls below remain supported.</p>
  ...
  },
  {
    ...
    body: (
      <>
        ...
        <p>For this separate, model-free server check, also install <code>@opencode-ai/sdk@1.15.13</code>. Run <code>node opencode-server.mjs</code>. It creates its own sandbox, starts the server with one shared password, verifies both authentication layers, reads configuration and cleans up. This GET-only check retains compatibility with older clients; use the <a href="#docs/typescript-sdk?section=protected-services">rc.11 scoped adapter</a> for the full Request contract.</p>
