...
test("installation pins reviewed artifacts, separates configuration and protects the target", () => {
  ...
  assert.equal(image, pins.opencodeImageIndex);
  assert.notEqual(image, pins.opencodeImage, "Cross-architecture guide must not select one architecture's child manifest");
...
test("cold-start evidence and capacity rollback limits are disclosed without promising a new release", () => {
  ...
  assert.match(markdown, /native execution and encrypted-recovery evidence/);
