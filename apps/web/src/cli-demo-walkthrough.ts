...
export const cliDemoSteps: DemoStep[] = [
  {
    ...
    text: 'A discount is missing from an invoice calculation. Give OpenCode a broken implementation and four existing acceptance tests in a disposable workspace. This demonstrates a developer delegating a small repair, not an agent deciding what correctness means. All inputs are synthetic.', language: "plaintext", code: '',
  ...
  },
  {
    ...
    text: 'The CLI runs on your machine and calls Harakiri. OpenCode, Node and Git execute inside the sandbox. Model inference is external: OpenCode sends task context to the selected provider. Your Harakiri API key stays in your local shell configuration, never in the task files.', language: "plaintext", code: '',
  ...
  },
  {
    ...
    language: "plaintext", code: cliDemoPrompt,
  ...
  },
  {
    ...
    text: 'The recording contains OpenCode reading the source and tests, editing the implementation, and running Node tests. The film replays reviewed facts from that execution. Tool order and implementation details can differ when you reproduce it.', language: "plaintext", code: '',
  ...
  },
  {
    ...
    language: "bash", code: 'harakiri run "$SBX" --cwd /workspace \\\n  --cmd "node --test invoice.spec.mjs"\nharakiri run "$SBX" --cwd /workspace \\\n  --cmd "sha256sum invoice.spec.mjs"\nharakiri run "$SBX" --cwd /workspace \\\n  --cmd "git diff -- invoice.mjs"',
  ...
  },
  {
    ...
    text: 'The recorded implementation applies the discount to subtotal, rounds to integer cents, then adds shipping. The caller confirmed all four original tests pass. This is evidence for this fixture, not proof against every possible invoice edge case.', language: "plaintext", code: '',
  ...
  },
  {
    ...
    language: "bash", code: 'harakiri kill "$SBX"\nharakiri inspect "$SBX"\n# Only after terminated is confirmed:\ntrap - EXIT',
