...
test("vision distinguishes runtime ownership, metadata, files and current limitations", () => {
  ...
  for (const phrase of ["OpenSandbox is the current integrated runtime provider", "not Kubernetes exec", "PostgreSQL", "metadata", "workspace volumes", "not an agent framework", "restricted OpenShift", "still pending", "0.5.0-rc.3"]) {
