...
try {
  for (const [name, width, height, reduced] of [['desktop', 1440, 1000, false], ['wide', 1920, 1080, false], ['mobile', 390, 844, false], ['narrow', 320, 568, false], ['reduced', 390, 844, true]] as const) {
    ...
    await page.getByRole('navigation', { name: 'Workflows' }).getByRole('link').nth(2).click();
    await expect(page).toHaveURL(/#demos\/sdk-agent-report$/);
    await page.goBack();
    await expect(page).toHaveURL(/#demos$/);
    await page.goForward();
    await expect(page.getByRole('heading', { name: demos[2]!.title, exact: true })).toBeVisible();
    for (const demo of demos) {
      ...
      const pixels = await video.evaluate((element: HTMLVideoElement) => {
        ...
        // Inspect the content panel, after the brief chapter reveal, at a
        // resolution that preserves text in light-background UI footage.
        ctx.drawImage(element, 0, 180, 1920, 768, 0, 0, 640, 256);
        const data = ctx.getImageData(0, 0, 640, 256).data;
        let dark = 0, light = 0;
      ...
      if (name === 'desktop' && demo.surface === 'CLI') {
  ...
  await expect(page.getByRole('heading', { name: 'CLI: repair code with OpenCode', exact: true })).toBeVisible();
