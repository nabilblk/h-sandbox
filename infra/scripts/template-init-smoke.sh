...
NAME="${HARAKIRI_TEMPLATE_INIT_SMOKE_NAME:-init-smoke-$(date +%s)}"
BUILD_TIMEOUT_SECONDS="${HARAKIRI_TEMPLATE_INIT_TIMEOUT_SECONDS:-300}"
...
CONTEXT_DIR="$(mktemp -d /tmp/harakiri-template-init-smoke.XXXXXX)"
CLI_HOME="$(mktemp -d /tmp/harakiri-cli-home.XXXXXX)"

cat >"${CONTEXT_DIR}/Dockerfile" <<'DOCKERFILE'
FROM ubuntu:24.04
RUN echo harakiri-init-built > /harakiri-init-built.txt
CMD ["sleep", "3600"]
DOCKERFILE
...
(
  ...
  HOME="${CLI_HOME}" HARAKIRI_API_URL="${API_URL}" HARAKIRI_API_KEY="${HARAKIRI_API_KEY}" \
    node "${CLI}" template init \
      ...
      --port 3000 \
      --tag hot \
      --runtime-family linux \
      --force \
      --ready-command "test -d /workspace" \
      >/dev/null
)

grep -q "id = \"${NAME}\"" "${CONTEXT_DIR}/harakiri.toml"
grep -q 'tags = \["hot"\]' "${CONTEXT_DIR}/harakiri.toml"
grep -q 'ready_command = "test -d /workspace"' "${CONTEXT_DIR}/harakiri.toml"

BUILD_OUTPUT="$(HOME="${CLI_HOME}" HARAKIRI_API_URL="${API_URL}" HARAKIRI_API_KEY="${HARAKIRI_API_KEY}" \
  node "${CLI}" template build "${CONTEXT_DIR}" --timeout "${BUILD_TIMEOUT_SECONDS}")"
printf '%s\n' "${BUILD_OUTPUT}"
BUILD_ID="$(printf '%s\n' "${BUILD_OUTPUT}" | awk '/^bld_/ {print $1}' | tail -1)"
if [[ -z "${BUILD_ID}" ]]; then
  echo "template init smoke build did not print a build id" >&2
  exit 1
fi
if ! grep -q -- "-> success. build=${BUILD_ID}" <<<"${BUILD_OUTPUT}"; then
  echo "template init smoke build ${BUILD_ID} did not report success" >&2
  exit 1
fi
if ! grep -q -- "runtime image pre-pull ok" <<<"${BUILD_OUTPUT}"; then
  echo "template init smoke build ${BUILD_ID} did not pre-pull the hot image" >&2
  exit 1
fi
...
if [[ -z "${SANDBOX_ID}" ]]; then
  echo "template init smoke sandbox create did not print a sandbox id" >&2
  exit 1
fi

RUN_OUTPUT="$(HOME="${CLI_HOME}" HARAKIRI_API_URL="${API_URL}" HARAKIRI_API_KEY="${HARAKIRI_API_KEY}" \
  node "${CLI}" run "${SANDBOX_ID}" --cmd "cat /harakiri-init-built.txt")"
printf '%s\n' "${RUN_OUTPUT}"
if ! grep -q "harakiri-init-built" <<<"${RUN_OUTPUT}"; then
  echo "sandbox did not run the image built from generated harakiri.toml" >&2
  exit 1
fi

echo "template init smoke passed: ${BUILD_ID} ${SANDBOX_ID}"
